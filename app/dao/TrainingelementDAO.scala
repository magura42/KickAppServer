...
@Singleton()
class TrainingelementDAO @Inject()(protected val dbConfigProvider: DatabaseConfigProvider)
  ...
  def getTrainingelement(trainingelementId: Int): Future[Option[Trainingelement]] = db
    .run(trainingelements.filter(_.trainingelementid === trainingelementId).result.headOption)

  def deleteTrainingelement(trainingelementId: Int): Future[Int] = db
    .run(trainingelements.filter(_.trainingelementid === trainingelementId).delete)

  def deleteAll(trainingId: Int): Future[Int] = db.run(trainingelements.filter(_.trainingid === trainingId).delete)
